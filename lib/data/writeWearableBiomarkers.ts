...
export async function writeWearableBiomarkers(
  ...
  // draft (AI narrative too) best-effort. Both self-guard on a signed/delivered
  // card (resyncDraftScores no-ops post-sign-off; regenerateDraft is state-gated),
  // so an automatic sync can never overwrite a clinician-signed card.
